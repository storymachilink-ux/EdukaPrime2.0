...
export default function ProductCard({ product, onClick, userPlan = 'completo', onDetailsClick, onUpsellClick }: ProductCardProps) {
  ...
  // Verificar se é produto Natal
  const isNatalProduct = ['5', '6', '7'].includes(product.id);
  const isNatalBasicoProduct = product.id === '5'; // Kit Básico

  // Determinar acesso para usuários Natal
  let hasNatalAccess = true;
  if (isNatalBasicPlan && !isNatalBasicoProduct) {
    hasNatalAccess = false; // user03natal só acessa Kit Básico
  }
  if (isNatalCompletoPlan && !isNatalProduct) {
    hasNatalAccess = false; // user76natal não acessa produtos fora de Natal
  }

  // Lógica original de acesso
  const isFreeProduct = product.id === '1'; // Turma EdukaBoo é liberada para plano básico
  const canAccess = (!isBasicPlan || isFreeProduct) && hasNatalAccess;
